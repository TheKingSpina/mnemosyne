...
import { DomainError, toDomainError, type MemoryService } from '@mnemosyne/core';
...
export function createApiServer(service: MemoryService): Server {
...
async function handleRequest(
  ...
  if (request.method === 'POST' && sessionClose) {
    ...
    bodylessSchema.parse(await readJson(request));
  ...
  if (request.method === 'POST' && url.pathname === '/v1/proposals') {
    sendJson(
      response,
      201,
      await service.proposeMemory(proposeMemoryInputSchema.parse(await readJson(request)), {
        actor: 'harness',
  ...
  if (request.method === 'POST' && url.pathname === '/v1/context/resolve') {
    sendJson(
      response,
      200,
      await service.resolveContext(contextInputSchema.parse(await readJson(request))),
    );
    return;
  }
  ...
  if (request.method === 'POST' && retraction) {
    ...
    const body = z.object({ reason: z.string().min(1).max(2_000) }).parse(await readJson(request));
  ...
  if (request.method === 'POST' && forget) {
    ...
    const body = z.object({ confirmationToken: z.string().min(1) }).parse(await readJson(request));
